...
export const DiseaseDetectionPage: React.FC = () => {
  ...
  // History state
  const [historyReports, setHistoryReports] = useState<DiseaseReport[]>([]);
  ...
  const handleAnalyzeDisease = async () => {
    ...
    const transitionTimer = (step: number, delay: number) => {
    ...
    try {
      ...
      const fullReport: DiseaseReport = {
        id: "rep_" + Date.now(),
  ...
  const handleReopenReport = (report: DiseaseReport) => {
